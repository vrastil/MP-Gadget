...
setup_glass(PetaPM * pm, double shift, int Ngrid, int seed, double mass, int NumPart, struct ic_part_data * ICP)
{
    ...
    idgen_init(idgen, pm, Ngrid, All.BoxSize);
    ...
    int i;
    // DISABLE omp RNG race condition
    ...
    for(i = 0; i < NumPart; i ++) {
        ...
        ICP[i].Pos[0] += shift + 3 * (gsl_rng_uniform(rng) - 0.5);
        ICP[i].Pos[1] += shift + 3 * (gsl_rng_uniform(rng) - 0.5);
        ICP[i].Pos[2] += shift + 3 * (gsl_rng_uniform(rng) - 0.5);
        ICP[i].Mass = mass;
    }
