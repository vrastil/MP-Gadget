...
void gravpm_force(ForceTree * tree) {
    ...
    if(ThisTask == 0)
        powerspectrum_save(&PowerSpectrum, All.PowerSpectrumDir, "powerspectrum", All.Time, GrowthFactor(All.Time, 1.0));
    if(ThisTask == 0 && All.MassiveNuLinRespOn)
        powerspectrum_nu_save(&PowerSpectrum, All.PowerSpectrumDir, "powerspectrum-nu", All.Time);
