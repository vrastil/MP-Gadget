...
int main(int argc, char **argv)
{
  ...
  if(All2.ProduceGas) {
    if(!All2.MakeGlassGas) {
        setup_grid(pm, shift_gas, All2.NgridGas, mass[0], NumPartCDM, ICP+NumPartCDM);
    } else {
        setup_glass(pm, 0, All2.NgridGas, GLASS_SEED_HASH(All2.Seed + 1), mass[0], NumPartCDM, ICP+NumPartCDM);
    }
